...
class CourseModel {
    ...
    async getCourseByName(name) {
        const query = 'SELECT id, name, lesson_count, img, description, status FROM course WHERE name = ?';
    ...
    async getCourseById(id) {
        const query = 'SELECT id, name, lesson_count, img, description, status FROM course WHERE id = ?';
        return await db.execute(query, [id]);
    }

    async updateCourse(id, name, lesson_count, img, description) {
        const query = 'UPDATE course SET name = ?, lesson_count = ?, img = ?, description = ? WHERE id = ?';
        return await db.execute(query, [name, lesson_count, img, description, id]);
    }

    async deleteCourse(id) {
        const query = 'UPDATE course SET status = 0 WHERE id = ?';
        return await db.execute(query, [id]);
    }
};
